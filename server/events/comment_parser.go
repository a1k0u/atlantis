...
const (
	...
	mergeWithSquashFlagLong      = "merge-with-squash"
	mergeWithSquashFlagShort     = ""
...
type CommentBuilder interface {
	...
	BuildApplyComment(repoRelDir string, workspace string, project string, autoMergeDisabled, mergeWithSquash bool) string
...
func (e *CommentParser) Parse(rawComment string, vcsHost models.VCSHostType) CommentParseResult {
	...
	var verbose, autoMergeDisabled, mergeWithSquash bool
	...
	case command.Apply.String():
		...
		flagSet.BoolVarP(&mergeWithSquash, mergeWithSquashFlagLong, mergeWithSquashFlagShort, false, "Enable squash method for automerge.")
		flagSet.BoolVarP(&verbose, verboseFlagLong, verboseFlagShort, false, "Append Atlantis log to comment.")
	...
	if autoMergeDisabled && mergeWithSquash {
		err := fmt.Sprintf("cannot use --%s at same time with --%s", mergeWithSquashFlagLong, autoMergeDisabledFlagLong)
		return CommentParseResult{CommentResponse: e.errMarkdown(err, cmd, flagSet)}
	}

	return CommentParseResult{
		Command: NewCommentCommand(dir, extraArgs, name, subName, verbose, autoMergeDisabled, mergeWithSquash, workspace, project, policySet, clearPolicyApproval),
	}
}
...
// BuildApplyComment builds an apply comment for the specified args.
func (e *CommentParser) BuildApplyComment(repoRelDir string, workspace string, project string, autoMergeDisabled, mergeWithSquash bool) string {
	flags := e.buildFlags(repoRelDir, workspace, project, autoMergeDisabled, mergeWithSquash)
...
func (e *CommentParser) buildFlags(repoRelDir string, workspace string, project string, autoMergeDisabled, mergeWithSquash bool) string {
	...
	if mergeWithSquash {
		flags = fmt.Sprintf("%s --%s", flags, mergeWithSquashFlagLong)
	}
	return flags
}
