...
type CommentCommand struct {
	...
	// MergeWithSquash is true if the command should use squash merge method in automerge.
...
func (c CommentCommand) String() string {
	return fmt.Sprintf("command=%q, verbose=%t, dir=%q, workspace=%q, project=%q, policyset=%q, auto-merge-disabled=%t, merge-with-squash=%t, clear-policy-approval=%t, flags=%q", c.Name.String(), c.Verbose, c.RepoRelDir, c.Workspace, c.ProjectName, c.PolicySet, c.AutoMergeDisabled, c.MergeWithSquash, c.ClearPolicyApproval, strings.Join(c.Flags, ","))
}

// NewCommentCommand constructs a CommentCommand, setting all missing fields to defaults.
func NewCommentCommand(repoRelDir string, flags []string, name command.Name, subName string, verbose, autoMergeDisabled, mergeWithSquash bool, workspace string, project string, policySet string, clearPolicyApproval bool) *CommentCommand {
	...
	return &CommentCommand{
		...
		MergeWithSquash:     mergeWithSquash,
		ProjectName:         project,
		PolicySet:           policySet,
