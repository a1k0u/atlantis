...
func TestProjectCommandContextBuilder_PullStatus(t *testing.T) {
	...
	t.Run("with project name defined", func(t *testing.T) {
		...
		When(mockCommentBuilder.BuildApplyComment(projRepoRelDir, projWorkspace, projName, false, false)).ThenReturn(expectedApplyCmt)
	...
	t.Run("with no project name defined", func(t *testing.T) {
		...
		When(mockCommentBuilder.BuildApplyComment(projRepoRelDir, projWorkspace, "", false, false)).ThenReturn(expectedApplyCmt)
		pullStatus.Projects = []models.ProjectStatus{
			{
