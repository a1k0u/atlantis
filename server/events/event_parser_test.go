...
func TestNewCommand_CleansDir(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.RepoRelDir, func(t *testing.T) {
			cmd := events.NewCommentCommand(c.RepoRelDir, nil, command.Plan, "", false, false, false, "workspace", "", "", false)
...
func TestNewCommand_AllFieldsSet(t *testing.T) {
	cmd := events.NewCommentCommand("dir", []string{"a", "b"}, command.Plan, "", true, false, false, "workspace", "project", "policyset", false)
	Equals(t, events.CommentCommand{
		Workspace:   "workspace",
		RepoRelDir:  "dir",
		Verbose:     true,
...
func TestCommentCommand_String(t *testing.T) {
	exp := `command="plan", verbose=true, dir="mydir", workspace="myworkspace", project="myproject", policyset="", auto-merge-disabled=false, merge-with-squash=false, clear-policy-approval=false, flags="flag1,flag2"`
